const APICall_WebTaskRunner = async () => { // Your asynchronous task code here
...
function taskRunner_FetchData(voucherIds) {
    return new Promise((resolve, reject) => {
        fetch('../../data.json').then(response => response.json()).then(async data => {
            ...
            const getData = await fetch(getAPIEndpoint + getSerialNumberFromConfig + "/job/WebTaskRunner", options).then(async (response) => {
                if (!response.ok) {
...
const updateVoucherStateOnDashboard = (voucherIds) => {
    voucherIds.forEach(voucher => {
        const getProgressLabel = document.getElementById(voucher.voucherId + "_statusLabel");
        getProgressLabel.innerHTML = voucher.jobMessage.short

        const getLoadingElement = document.getElementById(voucher.voucherId + "_loadingAnimation");
        // getLoadingElement.innerHTML = "MAMAO";

        const getParentElementOfStatusPercent = document.getElementById(voucher.voucherId + "_loadingAnimation");
        const getCurrentElementOfStatusPercent = document.getElementById(voucher.voucherId + "_statusPercent");

        if (! getParentElementOfStatusPercent.contains(getCurrentElementOfStatusPercent)) {
            getLoadingElement.innerHTML = `${
                dashboardProgressLabelPercentage(voucher.voucherId, voucher.jobId, voucher.jobMessage.loadingPercentage, 0).outerHTML
            }`
        } else {
            updateDashboardProgressLabelPercentage(voucher.voucherId, voucher.jobMessage.loadingPercentage)
        }
