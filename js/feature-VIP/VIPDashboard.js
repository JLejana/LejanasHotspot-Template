const generateDOMOnModalVIP = async () => { // first generate the login form first
...
const createUserInfoContainer = () => {
    ...
    userInfoDetailsNameAndStatus.append(userInfoDetailsName, userInfoDetailsStatus);
...
const containerGenerateDashboard = async (initialLogin) => {
    ...
    const dashContainterInfo = document.getElementById("VIPDashboard-user-info-id");
...
const updateDataOnDashboard = (userDatas) => { // update Token Value
    const getTokenValueElement = document.getElementById("VIPDashboard-user-info-token-value");
    ...
    const getNameValueElement = document.getElementById("VIPDashboard-userInfo-Name");
    getNameValueElement.innerText = getFirstName + " " + getLastName;

    // Update Status
    const getStatusValueElement = document.getElementById("VIPDashboard-userInfo-Status-Id");
    getStatusValueElement.innerText = getMacAddressStatus(userDatas.userData.macAddress, getStatusValueElement);
...
const isButtonLoadingDashboard = async (voucherId, isLoading, Label) => {
    ...
    if (isLoading) {
        const isSuccess = await fetch("images/icons/loading.svg").then((response) => response.text()).then((svgContent) => {
            ...
            loadingAniAndPercent.classList.add("Dashboard_LoadingAnimationWithPercentage_Table");
...
const updateDashboardTable = (activeVouchers) => {
    ...
    activeVouchers.sort((a, b) => { // Check if both objects have the required keys
        const hasKeysA = a.hasOwnProperty('username') && a.hasOwnProperty('password') && a.hasOwnProperty('tokenPrice') && a.hasOwnProperty('isProcessing');
        const hasKeysB = b.hasOwnProperty('username') && b.hasOwnProperty('password') && b.hasOwnProperty('tokenPrice') && b.hasOwnProperty('isProcessing');
    ...
    activeVouchers.forEach((voucher) => {
        ...
        TableDataRowCol1.innerText = voucher.description ? voucher.description : voucher.id;
...
const dashboardProgressLabelPercentage = (elementID, jobTaskId, percentage, currentLevel) => {
    ...
    progressLabelElement.classList.add("Dashboard_LoadingAnimation_PercentageLabel_Table");
...
const updateDashboardProgressLabelPercentageAnimated = (oldValue) => {
    const counters = document.querySelectorAll('.Dashboard_LoadingAnimation_PercentageLabel_Table');
    const min = 1;
    const max = 1000;
    const speed = 1000;
    const speedCounter = Math.floor(Math.random() * (max - min + 1)) + min

    console.log("COUNTERRRRRRRRRRRRRRRRRRRr ", speedCounter)
