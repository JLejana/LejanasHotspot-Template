const VIPAutoLogin = async () => {
    ...
    const autoLoginAPICall = async (options, data) => {
        ...
        const getData = await fetch("http://localhost:3000/api/" + getSerialNumberFromConfig + "/autoLogin", options).then((response) => {
            if (!response.ok) {
    ...
    if (getToken) {
        ...
        const getData = fetch('../../data.json').then(response => response.json()).then(data => {
            console.log(data);
...
function login(options, data, bodyData) {
    const getSerialNumberFromConfig = data.serialNumber;
    fetch("http://localhost:3000/api/" + getSerialNumberFromConfig + "/login", options).then((response) => {
        if (!response.ok) {
        ...
        if (data ?. isLogin) {
            localStorage.setItem("UserAccessToken", data.userData.user.stsTokenManager.accessToken);
...
const nextStepPassword = async () => {
    ...
    function getIfEmailIsExist(data, email) {
        ...
        fetch("http://localhost:3000/api/" + getSerialNumberFromConfig + "/isEmailExist?" + parameters, {method: "GET"}).then((response) => {
            if (!response.ok) {
                return response.json().then(errorData => {
                    throw new Error(errorData.errorMessage);
                });
    ...
    if (isEmailExist()) {
        ...
        var encryptedData = CryptoJS.AES.encrypt(JSON.stringify(dataToEncrypt), "Testing101").toString();
        const bodyData = {
            data: encryptedData
        };
        const options = {
    ...
    const loginCallAPI = async (options, data) => {
        ...
        const getData = await fetch("http://localhost:3000/api/" + getSerialNumberFromConfig + "/autoLogin", options).then((response) => {
            ...
            if (error ?. errorCode ?. code) {
                localStorage.removeItem("UserAccessToken");
                updateErrorMessage(convertErrorCodeToUserFriendly(error.errorCode.code));
            } else {
                updateErrorMessage("Something Went Wrong");
            } isButtonLoading(false, "Next");
        });
...
const dynamicallyGenerateHTMLForLogin = () => {
    ...
    if (container) {
        if (! container.contains(containerLabel)) { // this is for labeld Please Login Your Credential
...
const isButtonLoading = (isLoading, Label) => {
    ...
    if (isLoading) {
        fetch("images/icons/loading.svg").then((response) => response.text()).then((svgContent) => {
...
const updateTheDynamicallyGeneratedForPassword = () => {
    ...
    if (isEmailExist()) { // change the input from email to password
        ...
        const buttonContainer = document.getElementById("VIPLogin-Button-Container-id");
...
const backButtonFooterModal = (buttonState) => {
    ...
    switch (buttonState) {
        case "PASSWORD":
            ...
            break;
        default: backButton.hidden = true;
            buttonContainer.style = "justify-content: flex-end";
    }
};
