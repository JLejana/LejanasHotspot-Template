...
document.getElementById("modal-button-event").addEventListener("click", function () { // clear the error whenever they click the close button on modal
...
function AddItemsToTable(user) {
    ...
    td2.innerHTML = "<h1 class='tbl-username'>" + username + "</h1><p class='tbl-timeanddate'>" + statDate + "</p>";

    if (! isExpired) {
        td3.innerHTML = "<a href=http://" + domain + "/login?username=" + username + "&password=" + username + " class='button-like-link-active '>USE IT NOW!</a>";
    } else {
...
function formatDate(dateString) { // Create a Date object from the given string
    ...
    var formattedDate = month + " " + day + ", " + year + " " + hour + ":" + minute + " " + period;
...
const showErrorConnection = (isShown) => {
    ...
    if (isShown) {
        ...
        fetch("images/icons/alert-error.svg").then((response) => response.text()).then((svgContent) => {
...
const fetchData = (getStatus) => {
    ...
    fetchDataWithTimeout("https://mikrotik-hotspot-api.vercel.app/api/getHistory", 5000, options).then((response) => {
        if (!response.ok) {
